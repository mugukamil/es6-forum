...
_post.Post.findAll().then(function (res) {
    return res.json();
}).then(function (post) {
    return post.map(function (p) {
        return '\n  <article class="post">\n    <h2 class="post-title">' + _xssFilters2.default.inHTMLData(p.title) + '</h2>\n    <p class="post-meta">last reply on ' + _xssFilters2.default.inHTMLData(p.lastReply) + '</p>\n  </article>\n';
    });
}).then(function (post) {
    document.querySelector('section').innerHTML = post;
});

_post.Post.findRecent().then(function (res) {
    return res.json();
}).then(function (user) {
    return user.map(function (u) {
        return '\n    <div class="active-avatar">\n        <img width="54" src="assets/images/' + _xssFilters2.default.inHTMLData(u.avatar) + '">\n        <h5 class="post-author">' + _xssFilters2.default.inHTMLData(u.name) + '</h5>\n    </div>\n';
    });
}).then(function (html) {
    document.querySelector('.sidebar-content').innerHTML = html;
});

},{"./post":3,"xss-filters":1}],3:[function(require,module,exports){
'use strict';
